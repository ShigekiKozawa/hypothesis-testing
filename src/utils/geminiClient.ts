...
function getTimeoutForQuestionCount(count: number): number {
  if (count <= 10) return 180000;  // 3分
  if (count <= 20) return 360000;  // 6分
  if (count <= 30) return 480000;  // 8分
  return 600000;                   // 10分
}
...
export async function generateQuestions(
  ...
  const grade4Topics = `
【4級の出題範囲】
- データの種類（量的データ、質的データ）の識別
- 度数分布表（相対度数、累積度数、階級値）の読み取りと計算
- グラフの読み取り（ヒストグラム、箱ひげ図、円グラフ、帯グラフ、散布図、幹葉図、ドットプロット、折れ線グラフ、積み上げグラフ）
- 代表値（平均値、中央値、最頻値）の計算
- 散らばりの指標（範囲、四分位範囲、分散、標準偏差）の計算
- 偏差値の計算
- 確率の基本（独立事象、条件付き確率、復元抽出・非復元抽出）
- 割合・比率・指数の計算
- クロス集計表の読み取りと計算
- 公的統計の基礎（国勢調査の実施頻度など）
- 時系列データの読み取り（実数値と変化率の関係）
...
【統計検定の問題形式（絶対遵守）】
実際の統計検定では以下の2形式のみが出題されます：
1. 📊 表やグラフを読み取る問題（60-70%）
2. 🔢 データから計算する問題（30-40%）

🎯 必ず具体的なシチュエーション（状況設定）を含めること：
- 「あるクラス30人のテストの点数」
- 「A市とB市の気温データ」
- 「商品Xの売上推移」
- 「身長と体重の関係を調べた結果」
など、必ず実際の状況・文脈を設定してください。

❌ 絶対に作成してはいけない問題：
- 概念や用語の定義を選択する問題（例：「標準偏差とは何か」）
- 「～について正しいものはどれか」という抽象的な問題
- 図表がない問題（計算問題を除く）
- シチュエーション（状況設定）がない抽象的な問題

✅ 必ず作成すべき問題：
- ヒストグラム、箱ひげ図、散布図、度数分布表などを提示し、そこから値を読み取る問題
- 具体的な数値データ（例：「次の5人の点数 {60, 70, 80, 85, 85}」）を提示し、平均値、中央値、標準偏差などを計算させる問題
- グラフと表を組み合わせて読み取らせる問題
- すべての問題に具体的なシチュエーション（状況設定）を含める

# 良い問題の例（参考にすべきパターン）

【例1：グラフから読み取る問題（最も推奨）】✅
問題文: 「下のヒストグラムは、A中学校3年1組30人の数学のテストの点数分布を示しています。中央値が含まれる階級はどれですか。」
（ヒストグラムを必ず含める：chartType: "histogram", barData: [...]）
選択肢:
1. 60-70点
2. 70-80点 ✓（正解）
3. 80-90点
4. 90-100点
解説: 「全体の人数が30人で、中央値は15番目と16番目の平均値です。累積度数を計算すると、70-80点の階級に中央値が含まれます。」

【例2：データから計算する問題（推奨）】✅
問題文: 「あるクラスで5人の生徒を無作為に選び、1週間の読書時間を調査したところ、次のような結果でした。{60, 70, 80, 85, 85}分。この5人の読書時間の平均値を求めなさい。」
選択肢:
1. 72分
2. 76分 ✓（正解）
3. 80分
4. 82分
解説: 「平均値 = (60+70+80+85+85) ÷ 5 = 380 ÷ 5 = 76分」

【例3：散布図から読み取る問題（最も推奨）】✅
問題文: 「下の散布図は、10人の生徒について、1日の勉強時間（時間）と定期テストの成績（点）の関係を調べた結果を示しています。この散布図から読み取れる相関は次のうちどれですか。」
（散布図を必ず含める：chartType: "scatter", chartData: [...]）
選択肢:
1. 強い正の相関
2. 弱い正の相関 ✓（正解）
3. 相関なし
4. 負の相関
解説: 「散布図は右上がりの傾向を示しており、勉強時間が長いほど成績が高い傾向があります。しかし点のばらつきが比較的大きいため、弱い正の相関と判断できます。」

# 避けるべき悪い問題の例

【例：概念選択問題（絶対NG）】❌
❌ 問題文: 「標準偏差について正しい記述はどれか。」
理由: 図表がなく、抽象的な概念を選ばせる問題。実際の試験では出題されない。

【例：用語定義問題（絶対NG）】❌
❌ 問題文: 「相関係数の取りうる値の範囲として正しいものはどれか。」
理由: 図表がなく、用語の定義を問う問題。実際の試験では出題されない。

【例：図表のない抽象的な問題（絶対NG）】❌
❌ 問題文: 「外れ値が平均値に与える影響について正しいものはどれか。」
理由: 具体的なデータやグラフがない。必ず具体例を示す必要がある。

【例：範囲外の問題（4級）】❌
❌ 問題文: 「母集団から無作為抽出で標本を取る理由として正しいものはどれか。」
理由: 母集団・標本は4級の範囲外。

【例：範囲外の問題（3級）】❌
❌ 問題文: 「t検定を行う際の前提条件として正しいものはどれか。」
理由: t検定の詳細は3級の範囲外。
...
2. 誤答選択肢の設計（学習効果を高める）
   ...
   - グラフの読み取りミス（階級の読み間違い、軸の見間違いなど）
   - 似た統計量の取り違え（中央値と平均値、範囲と四分位範囲など）

3. 実践的なシナリオの使用
   - 身近な例：テストの点数、気温、身長体重、売上、年齢分布など
   - 実際の統計検定で出題されそうな状況設定
   - 必ず具体的な数値やグラフを伴う

4. 図表の作成
   - ヒストグラム：6-8個の階級、現実的な度数
   - 散布図：6-10個のデータ点、明確な相関傾向
   - 箱ひげ図：5数要約が明確に読み取れる
   - 度数分布表：4-7個の階級、合計20-50程度のデータ

5. 避けるべきパターン
   × 図表のない抽象的な問題
   × 概念や定義を問う問題
   × 「～について正しいものはどれか」という問題
   × 範囲外の高度な内容（4級：母集団・標本、3級：t検定・F検定・カイ二乗検定・ベイズ）
   × トリッキーな問題（揚げ足取り）
...
【${request.grade}の出題配分】
${request.grade === '4級' 
  ? '- グラフ読解問題: 50-60%（ヒストグラム、箱ひげ図、散布図、幹葉図など）\n- 度数分布表の計算問題: 15-20%\n- 確率の計算問題: 10-15%（具体的な状況設定必須）\n- データから代表値を計算する問題: 15-20%\n- 概念問題: 0%（絶対に出さない）'
  : '- 散布図から相関を読み取る問題: 15-20%（散布図必須）\n- ヒストグラム/箱ひげ図の読解: 15-20%\n- 度数分布表から統計量計算: 15-20%\n- データの変換と統計量の変化: 10-15%\n- 確率計算（二項分布など）: 10-15%\n- 信頼区間・仮説検定の基本（結果の解釈）: 10-15%\n- 標本調査の基礎: 5-10%\n- 概念問題: 0%（絶対に出さない）'
}
