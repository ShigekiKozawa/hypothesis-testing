...
【重要な制約】
⚠️ 図・グラフ・表は表示できません
- 「この図を見て」「グラフから読み取って」などの表現は使用しないでください
- データは必ず問題文中に数値として明記してください
  例：「散布図から相関を読み取る」× → 「データ (1,2), (2,4), (3,5), (4,7), (5,9) について相関係数を求めよ」○
  例：「ヒストグラムから最頻値を求めよ」× → 「度数分布表: 0-10点(5人), 10-20点(8人), 20-30点(12人)...から最頻値を求めよ」○
- グラフや表の特性を問う問題は、テキストと選択肢のみで完結させてください
...
【${request.grade}の特徴】
${request.grade === '4級' 
  ? '- データの種類と適切なグラフの選択問題を20%程度含める（グラフは表示せず、状況から判断させる）\n- 度数分布表の問題を15-20%程度含める（数値データを問題文に記載）\n- 確率の基本問題を10%程度含める\n- 具体的な数値を使った計算問題を多めに'
  : '- 推測統計の問題を25-30%程度含める\n- 相関・回帰分析を15-20%程度含める（データは数値で提示）\n- 仮説検定の問題を20-25%程度含める\n- 概念理解を問う問題と計算問題をバランスよく'
}
