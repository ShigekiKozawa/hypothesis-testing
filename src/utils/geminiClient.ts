...
6. 問題の多様性
   - 計算問題：具体的な数値を使った計算（30〜40%）
   - 概念問題：定義や特徴の理解を問う（30〜40%）
   - 応用問題：実際の場面での判断を問う（20〜30%）
...
【グラフを使う問題の例】
- 散布図：相関の強さや方向を視覚的に判断させる問題
- ヒストグラム：分布の形状（正の歪み、負の歪み）を判断させる問題
- 棒グラフ：カテゴリー間の比較や最大値・最小値を見つける問題
- 折れ線グラフ：時系列データの傾向を読み取る問題

⚠️ グラフなしでも解ける問題は、グラフを含めないでください
  例：「相関係数が0.8のとき、どのような相関があるか」→ グラフ不要
...
3. グラフを含める問題の割合
   - セクション問題（5-15問）: 2〜4問程度（20-30%）
   - 全分野に適用可能（散布図、ヒストグラム、折れ線グラフなど）
...
- 例：「1/4」と「13/52」は同じ値なので両方含めない → 「1/4」のみ使用
- 例：「1/2」と「2/4」は同じ値なので両方含めない → 「1/2」のみ使用
- 例：「1/3」と「2/6」は同じ値なので両方含めない → 「1/3」のみ使用
