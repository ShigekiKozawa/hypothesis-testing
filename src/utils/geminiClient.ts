...
【🔴 重点領域：採用度を増やす（失点が多い分野）】
以下の5つの領域は受験者の失点が特に多いため、問題生成時に優先的に採用してください。

1. **記述統計の「性質」を問う問題（採用度：高）**
   - 代表値の性質：外れ値がある場合、平均値は影響を受けるが中央値は影響を受けにくい
   - ヒストグラムの形状と代表値：右に裾が長い→最頻値<中央値<平均
   - 分散の公式： 分散 = (観測値の2乗和/n) - (平均値)²
   - 変動係数（CV）： 標準偏差 / 平均値（単位の異なるデータの散らばりを比較）
   - 累積相対度数グラフから中央値（0.5）や四分位数（0.25, 0.75）を読み取る

2. **2変数の関係：相関係数の計算と性質（採用度：高）**
   - 相関係数の計算： r = 共分散 / (xの標準偏差 × yの標準偏差)
   - 共分散の定義と計算手順
   - 散布図に直線的な傾向がない場合、r ≈ 0
   - 相関と因果の区別：相関≠因果（疑似相関の例：アイスと水難事故）
   - 1次変換と相関：a倍してb足しても相関係数の絶対値は不変（符号反転に注意）

3. **データの変換（1次変換）が統計量に与える影響（採用度：高）**
   - 平均値：a倍してb足すと、平均も a×平均+b になる
   - 標準偏差：b足しても不変、a倍すると |a|倍
   - 分散：a倍すると a²倍
   - 偏差値の不変性：全員に同じ1次変換を施しても、個人の偏差値は変わらない
   - 偏差値の公式： 50 + 10×(得点-平均)/標準偏差

4. **確率と時系列（採用度：高）**
   - 反復試行の確率： P(X=k) = nCk × p^k × (1-p)^(n-k)
   - 条件付き確率： P(B|A) = P(A∩B) / P(A)
   - 幾何平均（相乗平均）：年度比の平均は算術平均ではなく幾何平均を使う
   - 幾何平均の実戦的解法：選択肢をn乗して比較
   - 移動平均：データの平滑化、最初の数日は計算できない

5. **調査・実験・公的統計の用語（採用度：高）**
   - 母集団：調査対象の全体、標本の大きさ：n
   - 無作為抽出：どの個体も等しい確率で選ばれる（乱数表、くじ引き）
   - ×特定のクラスだけ選ぶのは無作為ではない
   - 実験計画法：比較したい要因以外は条件を揃える
   - 公的統計：家計調査、国勢調査、労働力調査→総務省

**問題生成時の優先ルール**：
- 上記5つの重点領域を含む問題を各セクションで最低30-40%含めること
- 特に「性質を問う問題」（なぜそうなるか、どういう影響があるか）を優先
- 計算だけでなく、意味や解釈を問う問題を多く含めること
`;
...
❌❌❌ 絶対に作成してはいけない問題（超重要）：
- 【概念問題は絶対禁止】概念や用語の定義を選択する問題（例：「標準偏差とは何か」「変動係数とは」「p値とは」）
- 【概念問題は絶対禁止】「～の性質として正しいものは」「～の意味として適切なものは」という問題
- 【概念問題は絶対禁止】公式や定理の名称を選ぶ問題（例：「この定理の名前は」）
- 「～について正しいものはどれか」という抽象的な問題
- 図表がない問題（計算問題を除く）
- シチュエーション（状況設定）がない抽象的な問題
- 【指示語は絶対禁止】「次の」「以下の」「上記の」「下記の」などの指示語を含む問題文

✅ 必ず作成すべき問題：
- ヒストグラム、箱ひげ図、散布図、度数分布表などを提示し、そこから値を読み取る問題
- 具体的な数値データ（例：「ある5人の点数 {60, 70, 80, 85, 85}」）を提示し、平均値、中央値、標準偏差などを計算させる問題
- グラフと表を組み合わせて読み取らせる問題
- すべての問題に具体的なシチュエーション（状況設定）を含める

📋 図表（chartType/chartData）が必要な問題と不要な問題：
✅ 図表が必要：グラフ読解、散布図、相関、回帰分析、ヒストグラム、箱ひげ図など
❌ 図表が不要：確率計算、データ変換（1次変換、標準化）、推測統計（信頼区間、仮説検定の判断）、変動係数の計算など
※図表が不要な問題では、chartType, chartData, barData, boxPlotData, chartLabels を一切含めないでください
